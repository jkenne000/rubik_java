...
public class Cube {
    ...
    public final static int[][] rotationAxes = { // 3 axes, 4 positions
    ...
    public final static boolean[][] rotationPolarity = { // 3 axes, 4 positions
	...
	exposedAxisCount = 0;
	for (i = 0; i < 3; i++) {
	    if (exposedAxis[i] == initExposedAxis[i] && exposedPolarity[i] == initExposedPolarity[i]) {
		matchCount++;
		tmpAxis[i] = 0;
	    }
	    if (exposedAxis[i] !=  -1) {
    ...
    public boolean isCube(int exposedAxis0, int exposedAxis1, int exposedAxis2, 
		boolean exposedPolarity0, boolean exposedPolarity1, boolean exposedPolarity2) {
     ...
     * @return true if cube is in original position, false otw
     */
    public boolean rotate(int axis, boolean polarity, boolean clockwise) throws Exception { 
		...
		//System.out.println("i = " + i);
			...
			//System.out.println("j = " + j);
                        if (rotationAxes[axis][index] == exposedAxis[axisToChange] &&
                            rotationPolarity[axis][index] == exposedPolarity[axisToChange]) {
                            if (leadingAxis == -1) {
                        ...
                        else if (rotationAxes[axis][index] == exposedAxis[i] &&
                            rotationPolarity[axis][index] == exposedPolarity[i]) {
                            if (leadingAxis == -1) {
                                leadingAxis = i;
                            }
                            else {
			    ...
			    //System.out.println("leadingAxis = " + leadingAxis);
	...
	//System.out.println("ROTATE");
        if (trailingAxis != -1) {
            ...
            exposedPolarity[trailingAxis] = (exposedPolarity[trailingAxis] ? false : true);
        }
        else if (axisToChange != -1) {
            // must be an edge (2 exposures), turn to next position
            CubeOrientation co = new CubeOrientation(exposedAxis[axisToChange], exposedPolarity[axisToChange]);
    ...
    public void dump() throws Exception {
        ...
        System.out.println(
         "axis = " + axisName(this.getAxis()) + " polarity = " 
         + this.getPolarity() + " rotation = " + this.getRotation());
        ...
        System.out.println("exposed axes = " 
                           + axisName(exposedAxis[0]) + " "
                           + axisName(exposedAxis[1]) + " "
                           + axisName(exposedAxis[2]));
        System.out.println("exposed pol  = "
                           + exposedPolarity[0] + " "
                           + exposedPolarity[1] + " "
                           + exposedPolarity[2]);
        System.out.println("");
    ...
    private class CubeOrientation {
        ...
        public void rotate(int turningAxis, boolean clockwise) throws Exception {
            ...
            for (j = 0; j < 4; j++) {
                if (rotationAxes[turningAxis][j] == this.axis &&
                    rotationPolarity[turningAxis][j] == this.polarity) {
                    if (clockwise) {
                        j++;
                        if (j == 4) j = 0;
                    }
                    ...
                    this.axis = rotationAxes[turningAxis][j];
                    this.polarity = rotationPolarity[turningAxis][j];
                    break;
                }
            }
            if (j == 4) {
